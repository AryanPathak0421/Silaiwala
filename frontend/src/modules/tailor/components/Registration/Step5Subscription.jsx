...
export const Step5Subscription = ({ register, watch, setValue }) => {
    ...
    const plans = [
        ...
        { id: 'premium', name: 'Premium', price: '₹999', features: ['Priority Search', 'Portfolio Badge', 'Advanced Analytics'], popular: true, color: 'from-primary to-primary-dark text-white' },
        ...
        <div className="space-y-8 animate-in slide-in-from-right duration-300 pb-10">
            ...
            <div className="space-y-4">
                {plans.map((plan) => (
                    <div
                        ...
                        className={`relative p-6 rounded-[2rem] border-2 transition-all cursor-pointer group ${selectedPlan === plan.id ? 'border-primary bg-gray-50/50 shadow-xl scale-[1.02]' : 'border-gray-100 hover:border-gray-200'
                            }`}
                    >
                        {plan.popular && (
                            <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border-2 border-white shadow-md">
                                Most Popular
                            </span>
                        )}
                        ...
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center transition-all ${selectedPlan === plan.id ? 'border-primary bg-primary text-white' : 'border-gray-200'
                                }`}>
                        ...
                        <ul className="space-y-3">
                            {plan.features.map((feature, idx) => (
                                <li key={idx} className="flex items-center gap-2 text-xs font-bold text-gray-500">
                                    <div className={`h-1.5 w-1.5 rounded-full ${selectedPlan === plan.id ? 'bg-primary' : 'bg-gray-300'}`}></div>
                                    {feature}
                                </li>
                            ))}
                        </ul>
            ...
            <div className="p-6 bg-[#f0f9f6] rounded-3xl border border-[#d4e9e2] mt-6">
                <div className="flex gap-3">
                    ...
                    <div>
                        <p className="text-xs font-black text-primary uppercase tracking-[0.1em]">Refund Policy</p>
